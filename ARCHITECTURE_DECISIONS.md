...
- **Rate limiting.** Not applied — removed by decision (2026-06). The API has no global limiter or per-endpoint policy. If reintroduced later, wire it via `AddRateLimiter` + `UseRateLimiter` and an `"auth"` policy on the credential/token endpoints.
- **Health checks.** `/health/live` (liveness) and `/health/ready` (DB connectivity via `ISqlConnectionFactory`), both anonymous.
