...
internal sealed class IncomeAnalysisReportGenerator(IDbExecutor db) : IReportGenerator
{
    public string ReportTypeKey => "IncomeAnalysis";
    ...
    {
        var (categories, months, totals) = await FetchDataAsync(userId, parameters, ct);
    ...
    private async Task<(IReadOnlyList<AnalysisCategoryRow>, IReadOnlyList<AnalysisMonthRow>, IncomeAnalysisTotals)>
        FetchDataAsync(long userId, ReportParameters p, CancellationToken ct)
    {
        ...
        return await db.QueryMultipleAsync(
            "MyMoney.usp_Report_IncomeAnalysis_GetData",
            async multi =>
            {
                var cats   = (await multi.ReadAsync<AnalysisCategoryRow>()).ToList();
                var months = (await multi.ReadAsync<AnalysisMonthRow>()).ToList();
                var totals = await multi.ReadSingleOrDefaultAsync<IncomeAnalysisTotals>()
                             ?? new IncomeAnalysisTotals();
                return (
    ...
    private static void BuildCategorySheet(
        ...
        IReadOnlyList<AnalysisCategoryRow> cats, IncomeAnalysisTotals totals)
    {
        bool ar  = lang == "ar";
        var  ws  = wb.Worksheets.Add(ar ? "الدخل حسب الفئة" : "Income by Category");
        int  row = 1;

        ws.Cell(row, 1).Value = ar ? "تحليل الدخل" : "Income Analysis";
        ...
        // Totals band
        ws.Cell(row, 1).Value = ar ? "إجمالي الدخل" : "Total Income";
        ...
        ExcelStyles.ApplyPositiveAmountStyle(ws.Cell(row, 2));
        ...
        IReadOnlyList<AnalysisMonthRow> months)
    {
        ...
        ws.Cell(row, 1).Value = ar ? "الدخل الشهري" : "Monthly Income";
        ...
        string[] headers = ar
            ? ["الشهر", "إجمالي الدخل", "عدد المعاملات"]
            : ["Month",  "Total Income",  "Transactions"];
